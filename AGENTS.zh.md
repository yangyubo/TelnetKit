...
- **已验证。** 在本机实际复现过：`swift test` 通过 105 个测试；`-strict-concurrency=complete` 无警告；`swift test --sanitize=address` 通过；`TelnetKit` 在五个平台下限（macOS 15、iOS 18、watchOS 11、tvOS 18、visionOS 2）均可构建；`leaks --atExit` 在 300 条连接与 10 万事件后报告 0 泄露字节。
...
swift test                        # 两个测试 target 共 105 个测试；离线且在 60s 内
