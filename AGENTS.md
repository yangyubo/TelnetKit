...
- **Verified.** Reproduced on this machine: `swift build` and `swift test` succeed with 97 tests, `swift build -Xswiftc -strict-concurrency=complete` reports no warning, `swift test --sanitize=address` passes, and `swift build --target TelnetKit --triple` succeeds for the macOS 15, iOS 18, watchOS 11, tvOS 18, and visionOS 2 floors.
...
swift test                        # 97 tests across both test targets; offline, under 60s
