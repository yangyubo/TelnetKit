...
2. 判定文档的职责：常驻规则、设计地图、调用方契约、需求、使用方契约，或工作流。[层级表](../../../docs/AGENTS.zh.md#层级分类一个事实一个家) 规定该职责的归属位置。
...
6. 用[字数上限](../../../docs/AGENTS.zh.md#字数上限)衡量文档，并按下沉—精简—上调的顺序处理。
...
- 中文对照版在同一次变更中更新，保持相同的标题序列、表格与代码结构以及物理行数；[PRD.zh.md](../../../PRD.zh.md) 与 [PRD.md](../../../PRD.md) 按与其他文档相同的规则配对。
...
2. 字数上限：`wc -w <file>` 在 [docs/AGENTS.md](../../../docs/AGENTS.zh.md#字数上限) 的上限之内，`AGENTS.md`、`docs/AGENTS.md`、`docs/architecture.md`、`docs/public-api.md` 需保留 5% 余量。
...
6. 测试方案：其中每条命令都实际跑过一次，每个套件都写明在哪些平台运行；在本机失败的命令连同错误一起记录，不得弱化。
7. 双语配对：两侧都存在，逐行的结构行类型一致，且 `wc -l` 报出的两侧行数相同。
8. `Sources/CLibTelnet/` 内没有任何手工编辑。
