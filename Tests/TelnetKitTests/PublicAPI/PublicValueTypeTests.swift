...
struct PublicValueTypeTests {
    ...
    @Test("standardClient offers BINARY, SGA, TERMINAL-TYPE, and NAWS and requests SGA and ECHO")
    func standardClient() {
        let options = TelnetOptions.standardClient
        #expect(options.local.map(\.option) == [.binary, .suppressGoAhead, .terminalType, .windowSize])
