...
The SwiftUI demo is a third manual entry point, and it is not a package target, so it builds from its own project under `Examples/`:

```sh
cd Examples/TelnetKitDemoApp
xcodebuild build -scheme TelnetKitDemoApp -destination 'generic/platform=macOS'
xcodebuild build -scheme TelnetKitDemoApp-iOS -destination 'platform=iOS Simulator,name=iPhone 17'
```

Its interactive checks are the manual acceptance steps in [PRD.md](../PRD.md#9-demo-project-requirements); [`Examples/TelnetKitDemoApp/README.md`](../Examples/TelnetKitDemoApp/README.md) carries the run procedure.
