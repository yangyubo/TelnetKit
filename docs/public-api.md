...
`standardClient` is `binary`, `suppressGoAhead`, and `terminalType` offered locally, and `suppressGoAhead` and `echo` requested remotely. `serverRequesting(_:)` builds the peer set a server-side caller needs.
