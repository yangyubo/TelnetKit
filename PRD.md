...
| L5 static assurance | Interface, concurrency, and five platform builds | `swift-api-digester` snapshot, `swift build -Xswiftc -strict-concurrency=complete`, an AddressSanitizer job, and the iOS, watchOS, tvOS, and visionOS simulator build and test matrix |

Every suite uses **Swift Testing** (`import Testing`, `@Test`/`@Suite`/`#expect`/`#require`) with `async` test functions; a test that needs timeout protection uses a `withTimeout` helper defined in the test target. The protocol suite runs on all five platforms; the public interface and integration suites, which bind a loopback listener, run on macOS and the iOS simulator.
...
| M5 demo | `telnetkit-client`, `telnetkit-echo-server`, and the SwiftUI demo app in `Examples/TelnetKitDemoApp/` | All four §9.3 criteria hold: the client reaches the echo server, a session against a real telnetd reached a shell, a window resize prints `NAWS 105x32` in the server log, the library passes 115 tests on an iOS 18 simulator and the app runs in the iOS simulator against the loopback server, and the app covers every public interface with a matching README snippet | Shipped |
| M6 Apple platform matrix | `Package.swift` declares all five platforms; CI gains iOS, watchOS, tvOS, and visionOS simulator builds and tests; the path events (FR-PATH) and background-suspension behavior are re-reviewed on iOS | All five platforms build (**verified: `swift build --target TelnetKit --triple` succeeds for the macOS 15, iOS 18, watchOS 11, tvOS 18, and visionOS 2 floors**); the protocol and public interface suites are green on macOS and iOS (**verified: 115 tests pass under `swift test` and again on the iPhone 17 simulator under `xcodebuild test`**), and the remaining platforms build | Shipped |
| M7 release | v0.1.0 tag, release notes, macOS and iOS simulator screenshots or recordings | The tag is pushed and `Package.resolved` is archived (**verified: the [CHANGELOG](CHANGELOG.md) carries the v0.1.0 notes, `Package.resolved` is tracked with its pins recorded there, both demo screenshots are committed under `docs/images/`, and the annotated `v0.1.0` tag exists locally; the push the exit criterion names is the human's publishing step**) | Partial |

> Status: **Shipped** means the exit criterion is met and the evidence is in [AGENTS.md](AGENTS.md#design-status); **Partial** means code or evidence has landed but the exit criterion is not met; **Planned** means no work has started. M6 holds its exit criterion: all five floors build, the 115-test suite is green on macOS and on the iPhone 17 simulator, the CI workflow carries the four-simulator matrix, and the remaining platforms build; the real cellular-to-Wi-Fi switch stays the device-only step in [docs/testing.md](docs/testing.md#manual-verification), and M5 holds all four §9.3 criteria. M7 is prepared, not shipped: the v0.1.0 release notes, the archived `Package.resolved` pins, and the two demo screenshots are committed and the annotated `v0.1.0` tag exists locally, while the criterion's `git push origin main --follow-tags` is reserved for the human, so M7 stays Partial.
...
| R11 | Five-platform CI cost and simulator resource use | Medium | iOS runs the full suite; watchOS, tvOS, and visionOS run the build plus the network-free suites (the vendored C suite and the protocol suite); the public interface and integration suites stay on macOS and the iOS simulator |
