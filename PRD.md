...
│   └── include/module.modulemap        # generated and untracked: module CLibTelnet { header "../libtelnet.h" export * }
├── PRD.md
├── README.md
├── LICENSE                             # this library's license (libtelnet is public domain; note it in NOTICE)
├── NOTICE
├── Sources/
│   ├── CLibTelnet/                     # the provenance record and the Swift seam, no upstream source
│   │   ├── UPSTREAM.md
│   │   └── UPSTREAM.zh.md
...
let package = Package(
    ...
    targets: [
        // libtelnet arrives as the libtelnet/ submodule, so the target path is the submodule root.
        // include/ is generated by ./.doc-tools/prepare-libtelnet.sh and stays untracked.
...
> A clone must run `git submodule update --init --recursive` and then `./.doc-tools/prepare-libtelnet.sh` to generate the module map.
...
```c
// libtelnet/include/module.modulemap, generated and untracked
module CLibTelnet {
    header "../libtelnet.h"   // resolved relative to the map, not to an include search path
    export *
}
```

- `libtelnet.c` and `libtelnet.h` come from the `libtelnet/` submodule at the pinned commit `5f5ecee` (version tag `\version 0.23`), and **no upstream file is modified**.
- The gitlink carries the pin and `Sources/CLibTelnet/UPSTREAM.md` records it; an upgrade checks out a new commit and re-runs the generator.
