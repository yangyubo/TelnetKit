...
| M5 demo | `telnetkit-client`, `telnetkit-echo-server`, and the SwiftUI demo app in `Examples/TelnetKitDemoApp/` | Criteria 1, 3, and 4 hold: the client reaches the echo server, a window resize prints `NAWS 105x32` in the server log, the library passes 115 tests on an iOS 18 simulator, and the app covers every public interface with a matching README snippet. Criterion 2 holds for a real telnetd, whose session reached a shell; its iOS-simulator clause is not recorded | Partial |
